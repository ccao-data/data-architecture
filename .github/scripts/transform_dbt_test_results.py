...
#
# Accepts four optional keyword arguments (options):
#
#   1. --run-results: The local path to a run_results.json file generated by a
#      `dbt test --store-failures` run
#        * If not present, defaults to './target/run_results.json'
#   2. --manifest: The local path to a manifest.json file with the compiled dbt
#      DAG, generated by all dbt CLI commands (including `dbt test`)
#        * If not present, defaults to './target/manifest.json'
#   3. --output-dir: The directory to which output artifacts should be written;
...
#        * If not present, defaults to all townships, including null townships
#          (which usually indicate invalid PINs)
#
...
#     should be written (defaults to s3://ccao-athena-results-us-east-1)
#  2. GIT_SHA: The SHA of the latest git commit (defaults to the output
#     of `git rev-parse`)
#  3. GIT_REF: The name of the ref for the latest git commit (defaults to
#     the output of `git rev-parse --abbrev-ref`)
#  4. GIT_AUTHOR: The author of the latest git commit (defaults to the output
#     of `git log`)
#
...
# root cause.
#
# Example usage with no options provided:
#
...
def main(run_results, manifest, output_dir, township) -> None:
    ...
    test_run_metadata = TestRunMetadata.create(
        run_results, run_by, git_sha, git_ref, git_author
    )
    test_run_result_metadata_list = TestRunResultMetadata.create_list(
        test_categories, run_results
    )
    run_date = get_run_date_from_run_results(run_results)
    run_id = get_run_id_from_run_results(run_results)
