...
FROM
  ((((((((((spatial.parcel pin
LEFT JOIN location.census ON ((pin.pin10 = census.pin10) AND (pin.year = census.year)))
LEFT JOIN location.census_acs5 ON ((pin.pin10 = census_acs5.pin10) AND (pin.year = census_acs5.year)))
LEFT JOIN location.political ON ((pin.pin10 = political.pin10) AND (pin.year = political.year)))
LEFT JOIN location.chicago ON ((pin.pin10 = chicago.pin10) AND (pin.year = chicago.year)))
LEFT JOIN location.economy ON ((pin.pin10 = economy.pin10) AND (pin.year = economy.year)))
LEFT JOIN location.environment ON ((pin.pin10 = environment.pin10) AND (pin.year = environment.year)))
LEFT JOIN location.school ON ((pin.pin10 = school.pin10) AND (pin.year = school.year)))
LEFT JOIN location.tax ON ((pin.pin10 = tax.pin10) AND (pin.year = tax.year)))
LEFT JOIN location.access ON ((pin.pin10 = access.pin10) AND (pin.year = access.year)))
LEFT JOIN location.other ON ((pin.pin10 = other.pin10) AND (pin.year = other.year)))
