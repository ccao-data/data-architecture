...
--
-- Returns columns with the prefix `len_` representing the length of each columns
-- for all rows where one of the columns has an incorrect length.
--
...
{% for column, length_column in length_columns %}
  ...
  {%- set filter_conditions = filter_conditions.append(
    '(' + length_column + ' is not null and ' + length_column + ' > ' + length|string + ')'
  ) %}
{% endfor %}
