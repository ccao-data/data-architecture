...
class TestResult:
    ...
    def details(self) -> str:
        ...
        """
        message = (
            f"- {self.table_name}: {self.description} ({self.status.value})"
        )
...
def main() -> None:
    ...
    # Generate the notification data and send it to each SNS topic. Start
    # by parsing out a message and subject for each group of failures
    ...
    for topic_arn, test_results in failures_by_topic.items():
        message = "The following iasWorld data tests are failing:"
        for test_result in test_results:
            message += f"\n\n{test_result.details}"
        message += (
            "\n\nThis notification was automatically generated by the Data "
            "Team. You will receive this notification once per day until the "
            "test failures are resolved. If you think you are receiving this "
            "notification incorrectly, contact Jean Cochrane or Dan Snow."
        )

        num_failing_rows = sum(len(res.failing_rows) for res in test_results)
        subject = (
            f"{num_failing_rows} data test"
            f"{'' if num_failing_rows == 1 else 's'} failed"
        )
