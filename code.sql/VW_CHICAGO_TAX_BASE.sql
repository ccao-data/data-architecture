...
FROM (
	...
	, X.TOTAL_BOR_EAV, TOTAL_CLERK_EXEMPTIONS + DIS_VET_80_EX AS TOTAL_CLERK_EXEMPTIONS FROM (  
	 ...
	 CL_DIS_VET_LT75K+CL_EXOWNEXVAL) AS TOTAL_CLERK_EXEMPTIONS
	  ...
	  GROUP BY H.TAX_YEAR, CL_TXCD, TOWN, LEFT(CLS, 1)
	 ) AS X
	LEFT JOIN
	...
	( SELECT H.TAX_YEAR
	  ...
	  , CASE WHEN LANDVAL*FACTOR+BLDGVAL*FACTOR <250000 THEN LANDVAL*FACTOR+BLDGVAL*FACTOR ELSE 250000 END AS DIS_VET_80_EX
	FROM BAMASTER AS H
	LEFT JOIN CLERKVALUES AS CLRK
