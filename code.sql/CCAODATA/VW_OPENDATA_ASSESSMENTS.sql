...
FROM 
(SELECT DISTINCT
	CASE WHEN A.TAX_YEAR IS NULL AND B.TAX_YEAR IS NOT NULL THEN B.TAX_YEAR
	WHEN A.TAX_YEAR IS NOT NULL THEN A.TAX_YEAR
	WHEN A.TAX_YEAR IS NULL AND B.TAX_YEAR IS NULL THEN 'JOIN ERROR'
		END AS TAX_YEAR
	, CASE WHEN A.PIN IS NULL AND B.PIN IS NOT NULL THEN B.PIN
	WHEN A.PIN IS NOT NULL THEN A.PIN 
	WHEN A.PIN IS NULL AND B.PIN IS NULL THEN 'JOIN ERROR' 
		END AS PIN
	FROM AS_HEADT AS A
	FULL OUTER JOIN DTBL_MODELVALS AS B
	ON A.PIN=B.PIN AND A.TAX_YEAR=B.TAX_YEAR
	WHERE A.PIN is NULL OR A.HD_CLASS IN (200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210, 211, 212, 234, 241, 278, 295, 299)
	) AS PIN_UNIVERSE
...
(
	SELECT UNIVERSE.PIN, UNIVERSE.TAX_YEAR AS YEAR, fitted_value_1 AS [MODEL RESULT]
	, fitted_value_6 AS [PIPELINE RESULT]
	, CASE WHEN UNIVERSE.version IS NULL THEN 0 ELSE UNIVERSE.version 
	END as VERSION
	...
	INNER JOIN
		...
		ON UNIVERSE.PIN = MAX_VERSION.PIN 
		AND UNIVERSE.TAX_YEAR = MAX_VERSION.TAX_YEAR 
		AND CASE WHEN UNIVERSE.version IS NULL THEN 0 ELSE UNIVERSE.version END = MAX_VERSION.VERSION
) FITTED_VALUES
