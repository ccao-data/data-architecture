/* 
This query produces a neat data set showing predicted market values for every PIN in 
Cook County for publication on the Cook Conty Open Data Portal. This data is also 
stored as a view in CCAO's internal server.
*/
...
SELECT
	...
	[MODEL RESULT],
	[PIPELINE RESULT],
	(HEADT.HD_ASS_BLD + HEADT.HD_ASS_LND) * 10 AS [MAILED],
	[CERTIFIED],
	(BOR.HD_ASS_LND + BOR.HD_ASS_BLD) * 10 AS [BOR RESULT],
	CASE WHEN [APPEALED] = 1 THEN 'YES'
		 ELSE 'NO'
		 END AS [CCAO APPEALED],
	CASE WHEN [NUM REVIEWED] IS NULL THEN 0 ELSE [NUM REVIEWED] END AS [NUM REVIEWED],
	CASE WHEN [CHANGED] = 1 THEN 'YES'
		 WHEN [CHANGED] = 0 OR [CHANGED] IS NULL THEN 'NO'
		 ELSE NULL
		 END AS [CCAO CHANGED],
	CASE WHEN BOR_APPEALS.PIN IS NULL THEN 'NO' ELSE 'YES' END AS [BOR APPEALED],
	CASE WHEN (BOR.HD_ASS_LND + BOR.HD_ASS_BLD) * 10 < [CERTIFIED] THEN 'YES' ELSE 'NO' END AS [BOR CHANGED]
FROM PIN_UNIVERSE
