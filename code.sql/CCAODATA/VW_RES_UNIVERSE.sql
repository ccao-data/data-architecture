...
, CASE WHEN DETAIL.DT_CLASS IN (200, 201, 241) OR (DETAIL.DT_CLASS = 299 AND (DT_CDU != 'GR' OR DT_CDU IS NULL) AND ((HD_PRI_BLD + HD_PRI_LND) > 10 OR (HD_PRI_BLD + HD_PRI_LND) IS NULL)) THEN 'NCHARS'
	WHEN DETAIL.DT_CLASS IN (202, 203, 204, 205, 206, 207, 208, 209, 210, 234, 278, 295) THEN 'SF'
	WHEN DETAIL.DT_CLASS IN (211, 212) THEN 'MF'
	WHEN DETAIL.DT_CLASS = 299 AND (DT_CDU = 'GR' OR (HD_PRI_BLD + HD_PRI_LND) <= 10) THEN 'FIXED'
...
LEFT JOIN
	...
	DT_PIN AS PIN, TB.TAX_YEAR, DT_AGE AS NCHARS_AGE, DT_CLASS, DT_CDU, DT_PER_ASS, CASE WHEN DT_KEY_PIN != 0 THEN RIGHT('0' + CAST(DT_KEY_PIN AS VARCHAR), 14) ELSE NULL END AS KEY_PIN
	FROM AS_DETAILTB AS TB
	LEFT JOIN
	CCAOSFCHARS AS C
	ON TB.PIN = C.PIN AND TB.TAX_YEAR = C.TAX_YEAR AND TB.DT_MLT_CD = C.MULTI_CODE
