...
, CASE WHEN DETAIL.DT_CLASS IN (200, 201, 241) OR (DETAIL.DT_CLASS = 299 AND (DT_CDU != 'GR' OR DT_CDU IS NULL) AND ((HD_PRI_BLD + HD_PRI_LND) > 3000 OR (HD_PRI_BLD + HD_PRI_LND) IS NULL) OR (HD_PRI_BLD + HD_PRI_LND) = 0) THEN 'NCHARS'
	WHEN DETAIL.DT_CLASS IN (202, 203, 204, 205, 206, 207, 208, 209, 210, 234, 278, 295) THEN 'SF'
	WHEN DETAIL.DT_CLASS IN (211, 212) THEN 'MF'
	WHEN DETAIL.DT_CLASS = 299 AND (DT_CDU = 'GR' OR ((HD_PRI_BLD + HD_PRI_LND) <= 3000 AND (HD_PRI_BLD + HD_PRI_LND) != 0))  THEN 'FIXED'
...
WHERE DT_CLASS IN (200, 201, 202, 203, 204, 205, 206, 207, 208, 209, 210, 211, 212, 234, 241, 278, 295, 299)