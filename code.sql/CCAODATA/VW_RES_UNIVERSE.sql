...
SELECT
	...
	CASE WHEN DETAILT.DT_CLASS IN (200, 201, 241) OR (DETAILT.DT_CLASS = 299 AND (DT_CDU != 'GR' OR DT_CDU IS NULL) AND (HD_PRI_BLD + HD_PRI_LND) NOT BETWEEN 1 AND 3000) THEN 'NCHARS'
		 WHEN DETAILT.DT_CLASS IN (202, 203, 204, 205, 206, 207, 208, 209, 210, 234, 278, 295) THEN 'SF'
		 WHEN DETAILT.DT_CLASS IN (211, 212) THEN 'MF'
		 WHEN DETAILT.DT_CLASS = 299 AND (DT_CDU = 'GR' OR (HD_PRI_BLD + HD_PRI_LND) BETWEEN 1 AND 3000) THEN 'FIXED'
		 ELSE NULL 
