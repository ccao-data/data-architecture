        ...
        FROM (
            ...
            FROM (
                ...
                INNER JOIN pin_locations o
                    ON ST_Contains(ST_Buffer(ST_Point(p.x_3435, p.y_3435), 100), o.point)
            ) dists
        )
        WHERE row_num <= 4
    )
...
-- Insert statement to fill in any missing PINs from the original CTAS
...
INSERT INTO proximity.dist_pin_to_pin_temp
WITH pin_locations AS (
    SELECT
        pin10,
        year,
