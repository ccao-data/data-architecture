...
WITH (
    ...
    external_location='s3://ccao-data-warehouse-us-east-1/proximity/dist_pin_to_water',
    ...
    hydrology_location AS (
        SELECT *
        FROM spatial.hydrology
    ),
    distances AS (
        SELECT
            p.x_3435,
        ...
        CROSS JOIN hydrology_location o
    ),
    xy_to_hydrology_dist AS (
        SELECT
            d1.x_3435,
            d1.y_3435,
            d1.id,
