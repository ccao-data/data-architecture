...
secondary_road_location AS (
    ...
    FROM (
        SELECT
            dy.year AS pin_year,
            '2022' AS fill_year
        FROM {{ source('spatial', 'secondary_road') }} AS df
        CROSS JOIN distinct_years AS dy
        WHERE dy.year >= df.year
    ) AS fill_years
    LEFT JOIN {{ source('spatial', 'secondary_road') }} AS fill_data
...
distances AS (
    ...
    CROSS JOIN secondary_road_location AS loc
),

xy_to_secondary_road_dist AS (
    SELECT
        d1.x_3435,
        d1.y_3435,
        d1.osm_id,
...
INNER JOIN xy_to_secondary_road_dist AS xy
