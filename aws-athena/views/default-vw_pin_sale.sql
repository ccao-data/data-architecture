...
-- 2021 doesn't have transaction numbers
dedupe_2021 AS (
    SELECT
        saledt, price,
        COUNT(*) AS dupes2021
    FROM iasworld.sales
    WHERE SUBSTR(sales.saledt, 1, 4) = '2021'
    GROUP BY saledt, price
),
unique_sales AS (
    SELECT DISTINCT
        sales.parid AS pin,
    ...
    LEFT JOIN dedupe_2021
        ON sales.price = dedupe_2021.price
        AND sales.saledt = dedupe_2021.saledt
    -- nopar is number of parcels sold
    WHERE ((sales.transno IS NOT NULL
            AND nopar = 1
            AND linenum = 1)
        OR obs = 1)
    AND (duplicate_transnos = 1 OR dupes2021 = 1)
