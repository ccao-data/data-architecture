WITH mydec_sales AS (
    SELECT
        ...
        COUNT(CASE WHEN LENGTH(buyer_name) < 2 OR buyer_name IS NULL THEN 1 END) AS buyer_null_count,
        COUNT(CASE WHEN LENGTH(seller_name) < 2 OR seller_name IS NULL THEN 1 END) AS seller_null_count
        FROM sale.mydec
