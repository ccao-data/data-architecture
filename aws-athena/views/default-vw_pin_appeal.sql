...
SELECT htpar.parid AS pin,
	...
	CASE
		...
		OR (htpar.user38 IN ('RS', 'IC')
		AND pardat.class IN ('213', '297', '299', '399', '599')) THEN 'condo/coop'
	...
	CASE
		...
		AND htpar.resact = 'C' THEN 'change'
		WHEN htpar.taxyr < '2020'
		AND htpar.resact = 'NC' THEN 'no change'
		WHEN htpar.taxyr >= '2020' THEN lower(htpar.user104) ELSE NULL
	END AS change,
	CASE
		WHEN htpar.taxyr < '2020'
		AND trim(substr(htpar.user42, 1, 2)) NOT IN ('0', ':') THEN trim(substr(htpar.user42, 1, 2))
		WHEN htpar.taxyr >= '2020' THEN htpar.user89 ELSE NULL
	END AS reason_code1,
	CASE
		WHEN htpar.taxyr < '2020'
		AND trim(substr(htpar.user43, 1, 2)) NOT IN ('0', ':') THEN trim(substr(htpar.user42, 1, 2))
		WHEN htpar.taxyr >= '2020' THEN htpar.user100 ELSE NULL
	END AS reason_code2,
	CASE
		WHEN htpar.taxyr < '2020'
		AND trim(substr(htpar.user44, 1, 2)) NOT IN ('0', ':') THEN trim(substr(htpar.user42, 1, 2))
...
FROM iasworld.htpar
	...
	LEFT JOIN default.vw_pin_value vwpv ON htpar.parid = vwpv.pin
	AND htpar.taxyr = vwpv.year
	LEFT JOIN iasworld.htagnt ON htpar.cpatty = htagnt.agent
WHERE htpar.cur = 'Y'
