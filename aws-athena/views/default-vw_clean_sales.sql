...
) TOWNS ON SALES.PARID = TOWNS.PARID AND SUBSTR(SALES.SALEDT, 1, 4) = TOWNS.TAXYR
...
) DROP_EXTRA_TRANSNO ON SALES.PARID = DROP_EXTRA_TRANSNO.PARID AND SALES.PRICE = DROP_EXTRA_TRANSNO.PRICE AND SALES.TRANSNO = DROP_EXTRA_TRANSNO.TRANSNO
...
) HIGHEST_SP ON SALES.PARID = HIGHEST_SP.PARID AND SALES.SALEDT = HIGHEST_SP.SALEDT AND SALES.PRICE = HIGHEST_SP.MAX_PRICE
...
WHERE sale_price_log10 BETWEEN lower_lim
  AND upper_lim