...
IDORSALES AS B
INNER JOIN
  (SELECT DISTINCT A.PIN, A.RECORDED_DATE, A.SALE_PRICE, MAX(A.DOC_NO) AS DOC_NO FROM
  IDORSALES AS A
  INNER JOIN
    (SELECT X.PIN, SS.RECORDED_DATE, MAX(X.SALE_PRICE) AS SALE_PRICE FROM
      IDORSALES AS X
      INNER JOIN
...
LEFT JOIN 
  ...
  GROUP BY LEFT(DT_PIN, 10), TAX_YEAR) AS BUILDING_UNITS /* Gives us ther percentage of ownership for Condos */
  ON LEFT(H.HD_PIN, 10)=BUILDING_PIN AND BUILDING_UNITS.TAX_YEAR=H.TAX_YEAR
LEFT JOIN
  (SELECT COUNT(HD_PIN) AS n_units, PIN10, TAX_YEAR FROM  /* gives us a count of the number of units in condo buildings */
