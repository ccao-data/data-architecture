...
RIGHT( LEFT(@AlLChars,ABS(BINARY_CHECKSUM(NEWID())%35) + 1 ),1) + RIGHT( LEFT(@AlLChars,ABS(BINARY_CHECKSUM(NEWID())%35) + 1 ),1) + 
RIGHT( LEFT(@AlLChars,ABS(BINARY_CHECKSUM(NEWID())%35) + 1 ),1) + '-' + RIGHT( LEFT(@AlLChars,ABS(BINARY_CHECKSUM(NEWID())%35) + 1 ),1)
AS RPIE_CODE, triad_name, FTBL_TOWNCODES.township_name, HD_NAME AS MAILING_NAME, PROPERTY_ADDRESS, PROPERTY_APT_NO, PROPERTY_CITY, PROPERTY_ZIP, MAILING_ADDRESS, MAILING_CITY, MAILING_ZIP, TAX_YEAR AS CALENDAR_YEAR
FROM (SELECT PIN,HD_TOWN, HD_NAME, HD_PRI_LND + HD_PRI_BLD as Assessed_Value, TAX_YEAR FROM AS_HEADT WHERE TAX_YEAR=2019 AND LEFT(HD_CLASS, 1) NOT IN ('0', '1','2', '4')) AS A
...
rpie_cleaned <- rpie %>%
  ...
  mutate(
    MAILING_STATE = MAILING_CITY %>% str_trim() %>% str_sub(-2, -1),
  ...
  mutate(MAILING_ZIP = ifelse(
    str_length(MAILING_ZIP) == 4,
    str_pad(MAILING_ZIP, 5, "left", 0),
    MAILING_ZIP)
  ) %>%
  mutate(
    RPIE_MAILING_ADDRESS = ifelse(str_length(MAILING_ADDRESS) == 0, PROPERTY_ADDRESS, MAILING_ADDRESS),
...
rpie_geocode_pass_2 <- rpie_geocode_pass_1 %>%
  separate(
    MAILING_ADDRESS_MATCHED,
    into = c("RPIE_MAILING_ADDRESS", "RPIE_MAILING_CITY",
             "RPIE_MAILING_STATE", "RPIE_MAILING_ZIP"),
    sep = ", "
  ) %>%
  mutate(
    RPIE_MAILING_ADDRESS = ifelse(is.na(RPIE_MAILING_ADDRESS), PROPERTY_ADDRESS, RPIE_MAILING_ADDRESS),
...
rpie_done <- rpie_cleaned %>%
  ...
  bind_rows(
    rpie_geocode_pass_2 %>% anti_join(rpie_geocode_pass_3 %>% select(RPIE_PIN)),
    rpie_geocode_pass_3
  )
