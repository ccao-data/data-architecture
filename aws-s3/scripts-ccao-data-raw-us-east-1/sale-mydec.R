...
files <- xml2::read_html('https://www2.illinois.gov/rev/localgovernments/property/Pages/mydecdatafiles.aspx') %>%
  html_nodes('a') %>%
  html_attr("href") %>%
  str_subset("ptax203")
...
down_up <- function(x) {
  ...
  if (!aws.s3::object_exists(file.path(output_bucket, glue("{year}.parquet")))) {
    ...
    # Grab zipped file - https://www2.illinois.gov/rev/localgovernments/property/Pages/mydecdatafiles.aspx
    download.file(glue("https://www2.illinois.gov{x}"), destfile = tmp1)
