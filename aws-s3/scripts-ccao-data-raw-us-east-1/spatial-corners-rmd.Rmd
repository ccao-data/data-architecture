...
DamonFunction_single_observation_final <- function(parcels, network, parcelsfull) {
 ...
 # Find out all the units touched by the cross
  touching_unit <- suppressWarnings(suppressMessages(st_intersects(cross$geom, clip_parcel[[i]])))
  
  # Step 3: Find out all the neighbor units for a parcel
  neighbor_unit <- suppressWarnings(suppressMessages(st_touches(parcel[[i]], clip_parcel[[i]])))
...
result <- DamonFunction_single_observation_final(parcels, network)

help(as_s2_geography)
...
result <- DamonFunction_single_observation(parcels, network)
