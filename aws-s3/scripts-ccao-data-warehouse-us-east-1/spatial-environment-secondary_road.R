...
# Difference method
#
dedup_data_union <- dedup_data %>%
  #mutate(geometry = st_buffer(geo))
  summarise(geometry = st_union(geometry))

major_roads_post_union <- major_roads_post %>%
  #select(geometry) %>%
  summarise((geometry = st_union(geometry)))

result <- st_difference(st_buffer(dedup_data_union, 1),
                        st_buffer(major_roads_post_union, 1))

#
# Intersection method
#
intersection_result <- st_intersection(dedup_data_union, major_roads_post_union)
data2_modified <- st_difference(dedup_data_union, intersection_result)

#
# Isolated Ashland differences / test buffer
#
new_bbox <- st_bbox(c(xmin = -87.68, ymin = 41.885,
                      xmax = -87.655, ymax = 42),
                    crs = st_crs(dedup_data))

subset_dedup_data <- st_intersection(dedup_data, st_as_sfc(new_bbox))
subset_major_roads_post <- st_intersection(major_roads_post, st_as_sfc(new_bbox))

subset_dedup_data_union <- subset_dedup_data %>%
  summarise((geometry = st_union(geometry)))
subset_major_roads_post_union <- subset_major_roads_post %>%
  summarise((geometry = st_union(geometry)))

result_subset <- st_difference(subset_dedup_data_union,
                               #subset_major_roads_prior_union
                               subset_major_roads_post_union)

result_subset <- st_difference(st_buffer(subset_dedup_data_union, 0.00001),
                                 st_buffer(subset_major_roads_post_union, 0.00001))
