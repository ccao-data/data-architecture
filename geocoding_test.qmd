...
missing_geographies_imputed <- missing_geographies %>%
  ...
  select(c("year", "pin10", "x_3435_imputed", "y_3435_imputed",
...
```{r}
geocoded_data <- geocoded_data %>%
  st_as_sf(coords = c("avg_lon_geocoded", "avg_lat_geocoded"),
           remove = FALSE) %>%
...
combined_geographies <- bind_rows(missing_geographies_imputed,
                                  geocoded_data) %>%
  st_cast("POINT") %>%
  st_transform(crs = 4326) %>%
  st_set_crs(4326) %>%
...
distance_by_group <- combined_geographies %>%
  ...
  st_cast("POINT") %>%
  mutate(distance_ft = st_distance(imputed_geom,
                                   geocoded_geom)[1, 1] * 3.28084) %>%
  ungroup()
