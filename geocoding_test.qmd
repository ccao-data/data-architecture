...
con <- dbConnect(noctua::athena())
  ...
  missing_geographies_imputed <- missing_geographies %>%
    ...
    select(c("year", "pin10", "x_3435_imputed", "y_3435_imputed", "lon_imputed", "lat_imputed", "prop_address_full"))
...
# Join the geocoded and imputed datasets
  final <- full_join(missing_geographies_imputed, geocoded_data, by = c("pin10", "year")) %>%
...
```{r}
geocoded_data <- geocoded_data %>%
  st_as_sf(coords = c("avg_lon_geocoded", "avg_lat_geocoded"), remove = FALSE) %>%
...
```{r}
geocoded_data <- geocoded_data %>%
  st_as_sf(coords = c("avg_lon_geocoded", "avg_lat_geocoded"), remove = FALSE, crs = 4326) %>%
...
combined_geographies <- bind_rows(missing_geographies_imputed, geocoded_data) %>%
  st_cast("POINT") %>%
  st_transform(crs = 4326) %>%
  st_set_crs(4326) %>%
  mutate(
...
distance_by_group <- combined_geographies %>%
  group_by(pin10, year) %>%
  summarise(
    imputed_geom = first(geometry[technique == "imputed"]),
    geocoded_geom = first(geometry[technique == "geocoded"]),
    address = first(prop_address_full)
  ) %>%
  filter(!is.na(imputed_geom) & !is.na(geocoded_geom)) %>%
  st_cast("POINT") %>%
  mutate(distance_ft = st_distance(imputed_geom, geocoded_geom)[1,1] * 3.28084) %>%
  ungroup()
